...
void LogIn::login(){
    ...
    while(t>0)//登录次数限制
    {
        ...
        {
            SetConsoleTextAttribute(tOut,FOREGROUND_GREEN|0X8);//TODO 模拟数据缓冲
            cout<<"\n\n\t\tLogging in...";
            symbol = "master";
            Sleep(1*1000);//TODO 模拟数据缓冲
            system("cls");
            break;
        }
...
void Master::showTheVersion(string *options,int optionNum,int thatIndex){
    ...
    HANDLE xOut;
    xOut = GetStdHandle(STD_OUTPUT_HANDLE);//得到控制句柄
...
void Master::readEmployeeDetail(){//将用户数据读入系统（程序）
    string empN;
    string empP;
    string empS;
    int intempS;
...
void Master::theMasterSystem(){
    ...
    while(true){
        readRestaurantData();//读数据

        showTheVersion(options,optionNum,thatIndex);//展示界面
        opo = select(optionNum,&thatIndex);//选择
        if(opo == ESC_){
            system("cls");
            cout<<"\n\n\n\t\t\t>>>>>>>>>>>>>>   This system is stop!  <<<<<<<<<<<<<<\n\n\n\n";
